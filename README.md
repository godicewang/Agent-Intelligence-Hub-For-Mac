...
FrostMI 当前界面只保留 **Agent Sensing** 一个主模块。它围绕 Mac 本机 AI Agent 的端上感知展开，默认进入 **Agent Analysis**，并提供 **Runtime Monitor** 和 **Static Scan** 两个下钻分页。Agent Analysis 顶部提供 **静态画像重建** 刷新按钮和 **运行时监控** 开关；下方按 **正在运行的 Agent** 与 **未运行的 Agent** 上下分栏展示，并按最近观察和使用信号排序。动态监控用轻量仪表盘展示运行中进程、FSEvents、权限与传感器状态；静态扫描负责发现 Agent、MCP Server、Skills、Context、Memory。Codex CLI 与 Codex App 会作为不同的本机 Agent 形态分别识别，避免桌面 UI 被 CLI 指纹吞并。
