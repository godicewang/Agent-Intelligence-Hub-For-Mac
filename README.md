...
FrostMI 当前界面只保留 **Agent Sensing** 一个主模块。它围绕 Mac 本机 AI Agent 的端上感知展开，拆分为 **Static Scan**、**Runtime Monitor** 和 **Agent Analysis** 三个置顶分页：静态扫描负责发现 Agent、MCP Server、Skills、Context、Memory；动态监控用轻量仪表盘展示运行中进程、FSEvents、权限与传感器状态；Agent 分析用 Agent 卡片把静态资产和运行时证据聚合成可点击画像。Codex CLI 与 Codex App 会作为不同的本机 Agent 形态分别识别，避免桌面 UI 被 CLI 指纹吞并。
...
The current FrostMI interface exposes **Agent Sensing** as the only primary module. It is centered on endpoint-native sensing for local AI agents on macOS and now has three top-level tabs: **Static Scan**, **Runtime Monitor**, and **Agent Analysis**. Static Scan discovers local Agents, MCP servers, Skills, Context files, and Memory assets. Runtime Monitor uses a lightweight live dashboard for running-process snapshots, NSWorkspace running-app attribution, FSEvents status, and permission states without fabricating Endpoint Security or Network Extension telemetry. Agent Analysis rolls static assets and runtime evidence into clickable agent cards. Codex CLI and Codex App are detected as separate local agent surfaces so the desktop UI is not collapsed into the CLI fingerprint.
