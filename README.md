...
FrostMI 当前界面只保留 **Agent Sensing** 一个主模块。它围绕 Mac 本机 AI Agent 的端上感知展开，默认进入 **Agent Analysis**，并提供 **Runtime Monitor** 和 **Static Scan** 两个下钻分页。Agent Analysis 顶部提供 **静态画像重建** 刷新按钮和 **运行时监控** 开关；下方按 **正在运行的 Agent** 与 **未运行的 Agent** 上下分栏展示，并按最近观察和使用信号排序。动态监控用轻量仪表盘展示运行中进程、FSEvents、权限与传感器状态，并直接呈现本地 runtime event store 的事件时间线和可展开的 Session Graph 摘要；重复网络快照和同一路径文件变化会合并为最新观察，避免长期运行产生无意义噪声。静态扫描负责发现 Agent、MCP Server、Skills、Context、Memory。Codex CLI 与 Codex App 会作为不同的本机 Agent 形态分别识别，避免桌面 UI 被 CLI 指纹吞并。

| 界面区域 | 当前作用 |
| --- | --- |
| Static Scan | 真实静态发现，发现 Claude Code、Cursor、Codex CLI、Gemini CLI、MCP、Skills、Context 和 Memory。 |
| Runtime Monitor | 真实动态监控，使用进程快照、NSWorkspace 运行 App 视图、FSEvents、轻量本机网络流快照、runtime event store、Session Graph 摘要和权限状态；重复快照会降噪保留，不伪造 Endpoint Security / Network Extension 遥测。 |
...
- Single-module endpoint intelligence shell focused on Agent Sensing with three tabs ordered as Agent Analysis, Runtime Monitor, and Static Scan.
